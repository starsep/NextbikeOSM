...
class Feed:
    def __init__(self, objname, nodes, ways, places):
        ...
        self.envir = Environment(loader=PackageLoader("feed_gen", "templates"))
        self.osm_new = []
        self.osm_rem = []
        self.osm_changes = []
        self.nxtb_new = []
    ...
    def new_db(self):
        ...
        if "{0}_FEED".format(self.objname) not in tables:
            c.execute(
                """CREATE TABLE {0}_FEED (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, data TEXT, content TEXT)""".format(
                    self.objname
                )
            )
        connection.commit()
    ...
    def check_db(self):
        ...
        dbaza = c.execute("""SELECT id FROM {0}_OSM""".format(self.objname)).fetchall()
        ...
        dbaza = c.execute("""SELECT id FROM {0}_NXTB""".format(self.objname)).fetchall()
        baza = self.detuple(dbaza)
    ...
    def create_feed(self):
        ...
        feeds = c.execute("""SELECT * FROM {0}_FEED""".format(self.objname)).fetchall()
        connection.close()
        ...
        for i in feeds:
            new_entry = {
                "title": i[1],
                "id": "{0}:{1}".format(self.objname, str(i[0])),
                "updated": i[2],
                "content": i[3],
            }
        ...
        fill_template = template.render(
            {
                "title": "Atom feed for {0}".format(self.objname),
                "name": "http://javnik.tk/NextbikeOSM/{0}_atom.xml".format(
                    self.objname
                ),
                "update": timek,
                "articles": articles,
